...
cmap <- plot_embeddings(myobj, savedest, cell_model='monocle2', w=6,h=4)
```
...
```{r echo=T, results = 'hide'}
plot_heatmaps(myobj, savedest, cell_model='monocle2', selected_genes=heatmap_genes) # if heatmap_genes is not specified, all genes (i.e. all columns of myobj@data_aggregate) will be used
```
...
plot_sample_histograms(myobj, group_assignments, savedest, cell_model='monocle2', cmap)
```
...
plot_summary_histograms(myobj, group_assignments, savedest, cell_model='monocle2', cmap)
```
