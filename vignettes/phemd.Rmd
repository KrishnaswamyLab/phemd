...
plotGroupedSamplesDmap(my_distmat, group_assignments, dest=NULL, 
                          pt_sz = 1.5)
```
...
To plot the cell subtype distribution on a per-sample basis, use the following 
plotting function.
...
Note that the *saveSampleHistograms()* function saves each histogram as a 
separate file in the *savedest* output folder rather than plotting them in a
single plot (due to an often large number of samples).
...
plotSummaryHistograms(myobj, group_assignments, savedest, 
                      ...
                      ax.lab.sz=1.3, title.sz=2)
```

Note that the *plotSummaryHistograms()* function optionally saves each 
histogram as a separate file in the *savedest* output folder.
